...
let fetch_real_input ~day ~year =
  ...
  let uri =
    Uri.of_string
      [%string
        "https://adventofcode.com/%{Int.to_string year}/day/%{Int.to_string day}/input"]
  in
...
module Submission_result = struct
  ...
  let of_body body =
    if String.is_substring body ~substring:"That's the right answer!"
    then Correct
    else if
      String.is_substring
        body
...
let submit_solution ~day ~year ~part ~answer =
  let headers =
    Cohttp.Header.of_list
      [ "Cookie", session_cookie; "Content-Type", "application/x-www-form-urlencoded" ]
  in
  let body =
    Cohttp_async.Body.of_string [%string "level=%{Int.to_string part}&answer=%{answer}"]
  in
  let uri =
    Uri.of_string
      [%string
        "https://adventofcode.com/%{Int.to_string year}/day/%{Int.to_string day}/answer"]
  in
