...
let do_stuff s fn john =
  ...
  |> List.map ~f:(fun char_num ->
    List.fold (List.range 0 26) ~init:('a', john) ~f:(fun (best_char, most_best) c ->
      if fn res.(c).(char_num) most_best
      then Char.of_int_exn (c + Char.to_int 'a'), res.(c).(char_num)
      else best_char, most_best)
    |> fst)
  |> String.of_list
;;

let part1 s = do_stuff s (fun a b -> b < a) Int.min_value
