...
let rec stuff p1_pos p1_score p2_pos p2_score turn =
  ...
  if Hashtbl.mem memo k
  then Hashtbl.find_exn memo k
  else if p1_score >= 21
  then (
  ...
  then (
    ...
    let res =
      Map.fold score_counts ~init:(0, 0) ~f:(fun ~key:moves ~data:count acc ->
        ...
        Coordinate.add
          acc
          (Coordinate.scale (stuff p1_pos' p1_score' p2_pos' p2_score' (not turn)) count))
    in
