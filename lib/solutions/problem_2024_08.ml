  ...
  let is_spot_antinode2 ({ antennas; _ } as t) pos =
    Map.exists antennas ~f:(fun s ->
      Set.exists s ~f:(fun antenna ->
        ...
        let rec aux pos =
          if not (is_in_range t pos)
          then false
          else if (not (Coordinate.equal antenna pos)) && Set.mem s pos
          then true
...
let part1 s =
  ...
  List.count (Map.keys t.board) ~f:(City.is_spot_antinode t) |> Int.to_string
;;

let part2 s =
  let t = City.of_string s in
  List.count (Map.keys t.board) ~f:(City.is_spot_antinode2 t) |> Int.to_string
;;
