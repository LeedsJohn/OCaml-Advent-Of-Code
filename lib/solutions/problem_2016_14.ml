...
let find_n_keys salt n hashfn =
  let get_new_keys open_stuff s =
    Map.filteri open_stuff ~f:(fun ~key:c ~data:_ -> has_run_of_n s c 5) |> Map.to_alist
  in
  ...
  let rec aux acc open_stuff i last_thing =
    if i - 1000 >= last_thing
    then acc
    else (
      let last_thing =
        if List.length acc >= n && last_thing = Int.max_value
        then List.max_elt acc ~compare:Int.compare |> Option.value_exn
        else last_thing
      in
      let open_stuff =
      ...
      let open_stuff = filter_open_stuff open_stuff (List.map new_keys ~f:fst) in
      let open_stuff =
  ...
  in
  let res =
    aux [] (Map.empty (module Char)) 0 Int.max_value |> List.sort ~compare:Int.compare
  in
  List.take res n
;;

let part1 s =
  let hashfn salt i = Md5.digest_string (salt ^ Int.to_string i) |> Md5.to_hex in
  find_n_keys s 64 hashfn |> List.last_exn |> Int.to_string
;;
