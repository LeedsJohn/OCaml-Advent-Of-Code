...
module Garden = struct
  ...
  let of_string s scale =
    ...
    in
    let s =
      List.fold s ~init:[] ~f:(fun acc line -> List.init scale ~f:(fun _ -> line) @ acc)
      |> List.rev
      |> String.concat_lines
    in
  ...
  let to_string { board; _ } group =
    Map.mapi board ~f:(fun ~key:pos ~data:c -> if Set.mem group pos then '.' else c)
    |> Board.to_string
  ;;
  ...
  let get_perimeter ({ board; scale } as t) start_pos =
    ...
    Set.sum
      ...
      ~f:(fun pos ->
        Set.count
          (Coordinate.neighbors pos |> Set.of_list (module Coordinate))
          ~f:(fun pos -> not (Char.equal (get t pos |> Option.value ~default:'.') plant)))
    / scale
  ;;
  ...
  let is_corner group pos =
    let out_corner =
      ...
      && not (Coordinate.equal (Set.fold neighbors ~init:(0, 0) ~f:Coordinate.add) (0, 0))
    in
    let in_corner =
      Set.count
