...
module Warehouse2 = struct
  ...
  let of_string s =
    ...
    let walls, boxes, pos =
      List.foldi
        s
        ~init:(Set.empty (module Coordinate), Map.empty (module Coordinate), (0, 0))
        ~f:(fun y acc line ->
          List.foldi line ~init:acc ~f:(fun x (walls, boxes, pos) c ->
            ...
            | '[' ->
              let l = x, y in
              let r = x + 1, y in
              walls, Map.add_exn boxes ~key:l ~data:r |> Map.add_exn ~key:r ~data:l, pos
            | '.' | ']' -> walls, boxes, pos
            | c -> raise_s [%message "invalid character for warehouse" (c : char)]))
    in
    { walls; boxes; pos }
  ;;
  ...
  let _to_string { boxes; walls; pos } =
    ...
    Map.fold boxes ~init:board ~f:(fun ~key:pos ~data:neighbor acc ->
      if Coordinate.(equal (add pos (1, 0)) neighbor)
      then Map.add_exn acc ~key:pos ~data:'[' |> Map.add_exn ~key:neighbor ~data:']'
      else acc)
    |> Board.to_string
  ;;
  ...
  let shuffle_boxes ({ walls; boxes; pos } as t) dir =
    let rec get_shuffled_boxes cur_level prev_boxes =
      ...
      | None -> None
      | Some next_level ->
        if Set.length next_level = 0
        then Some prev_boxes
    ...
    | Some shuffled_boxes ->
      let boxes =
        Map.mapi boxes ~f:(fun ~key:pos ~data:neighbor ->
          if Set.mem shuffled_boxes pos then Coordinate.add neighbor dir else neighbor)
        |> Map.map_keys_exn
             (module Coordinate)
             ~f:(fun pos ->
               if Set.mem shuffled_boxes pos then Coordinate.add pos dir else pos)
      in
...
let part1 s =
  ...
  Warehouse.apply_moves warehouse dir_list |> Warehouse.gps_coords |> Int.to_string
;;

let part2 s =
  let warehouse, dir_list = get_warehouse_and_dir_list s Warehouse2.of_string in
  Warehouse2.apply_moves warehouse dir_list |> Warehouse2.gps_coords |> Int.to_string
;;
