...
let is_valid line =
  List.for_all [ "byr"; "iyr"; "eyr"; "hgt"; "hcl"; "ecl"; "pid" ] ~f:(fun field ->
    String.is_substring line ~substring:(field ^ ":"))
;;
...
let hcl line =
  ...
  | 7 ->
    String.for_alli line ~f:(fun i c ->
      if i = 0 then Char.equal c '#' else String.contains "0123456789abcdef" c)
  | _ -> false
;;
...
let part1 s = passport_lines s |> List.count ~f:is_valid |> Int.to_string
let part2 s = passport_lines s |> List.count ~f:is_valid2 |> Int.to_string
