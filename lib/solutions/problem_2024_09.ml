...
module Entry_list = struct
      ...
      | (Full (n1, id1) as hd) :: tl ->
        (match acc with
         | Entry.Full (n2, id2) :: tl2 ->
           if id1 = id2 then aux (Full (n1 + n2, id1) :: tl2) tl else aux (hd :: acc) tl
         | _ -> aux (hd :: acc) tl)
    in
    let t = aux [ List.hd_exn t ] (List.tl_exn t) in
    List.filter t ~f:(function
      | Empty 0 | Full (0, _) -> false
      | _ -> true)
    |> List.rev
    |> List.drop_while ~f:(function
      | Entry.Empty _ -> true
      | _ -> false)
    |> List.rev
  ;;
  ...
  let of_string s : t =
    let rec aux acc i id =
      ...
      then
        aux (Entry.Full (Char.get_digit_exn (String.get s i), id) :: acc) (i + 1) (id + 1)
      else aux (Entry.Empty (Char.get_digit_exn (String.get s i)) :: acc) (i + 1) id
    in
    aux [] 0 0 |> fix_stuff
  ;;
  ...
  let shuffle_last_block t =
    ...
    let id, count =
      ...
      | Empty _ -> raise_s [%message "shouldn't be possible to get an empty thing here"]
    in
    let rec aux acc count = function
      | [] -> if count = 0 then List.rev acc else List.rev (Entry.Full (count, id) :: acc)
      | (Entry.Full _ as hd) :: tl -> aux (hd :: acc) count tl
      | Empty n :: tl ->
        if count = n
        then aux (Full (count, id) :: acc) 0 tl
        else if count > n
        then aux (Full (n, id) :: acc) (count - n) tl
  ...
  let shuffle_last_block' t =
    let count, id, t =
      List.fold (List.rev t) ~init:(-1, -1, []) ~f:(fun (count, id, acc) t ->
        match t with
        | Entry.Full (c, i) as e ->
          if id = -1 && i >= 0 then c, i, Entry.Empty c :: acc else count, id, e :: acc
        | e -> count, id, e :: acc)
    in
    let rec aux acc added = function
      ...
      | (Empty n as hd) :: tl ->
        if (not added) && n >= count
        then aux (Empty (n - count) :: Full (count, -id) :: acc) true tl
  ...
  let has_gap t =
    List.exists (fix_stuff t) ~f:(function
      | Empty _ -> true
      | Full _ -> false)
  ;;
...
let part1 s =
  ...
  let rec aux t =
    if Entry_list.has_gap t
    then aux (Entry_list.shuffle_last_block t)
