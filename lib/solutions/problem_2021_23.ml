...
module Pos = struct
  ...
  let of_int n =
    if Int.between n ~low:0 ~high:10 then Hallway n else Room ((n - 11) / 2, (n - 11) % 2)
  ;;
...
module Burrow = struct
  ...
  let rec is_pos_solved t pos =
    ...
    | Room (n, 0) -> Amphipod.goal_room a = n && is_pos_solved t (Pos.Room (n, 1))
    | _ -> false
  ;;

  let to_string t =
    let hallway =
      List.map (List.range 0 11) ~f:(fun i -> get t (Pos.Hallway i) |> Amphipod.to_char)
      |> String.of_list
    in
    let rooms1 =
      ...
      then ()
        ...
        let next_steps =
          ...
          |> List.filter ~f:(fun pos ->
            ...
            | _ -> false)
        in
        List.iter next_steps ~f:(fun key -> Hashtbl.add_exn visited ~key ~data:steps);
  ...
  let valid_end_pos_from_room t amph end_pos =
    ...
    | Pos.Room (n, buried_depth) ->
      if Amphipod.goal_room amph <> n
      then false
      else if buried_depth = 1
      then true
...
let djikstra t =
  ...
  let rec aux pq =
    ...
    if pos = Burrow.goal_state
    then energy
    else (
      let pq =
        List.fold (Burrow.all_moves pos) ~init:pq ~f:(fun acc (start_, end_, steps) ->
          let new_state = Burrow.apply_move pos start_ end_ in
          if Hash_set.mem visited new_state
          then acc
          else Pq.add acc (energy + Burrow.move_cost pos start_ steps, new_state))
      in
      aux pq)
  in
...
let%expect_test "places can go" =
  ...
  print_s [%sexp (List.length (Burrow.all_moves burrow) : int)];
  [%expect {| 18 |}];
  print_s [%sexp (Burrow.places_can_go burrow (Pos.Room (0, 1)) : (Pos.t * int) list)];
  [%expect {| () |}];
  ()
;;
