...
type bit_func = Xor | And | Or [@@deriving sexp]
...
module System = struct
  ...
  let rec get ({ wires; gates; _ } as t) wire =
    ...
    | Some v -> v
    | None ->
        if Wire.is_input wire then false
        else
          let in1, in2, _, f =
            List.find_exn gates ~f:(fun (_, _, out, _) -> out = wire)
          in
  ...
  let of_string s =
    ...
    let initial_values =
      List.map initial_values ~f:(fun line ->
          ...
          let data =
            Char.equal (String.get line (String.length line - 1)) '1'
          in
          (wire, data))
    in
    let gates =
  ...
  let get_matching_node { gates; _ } i f =
    ...
    List.find_map gates ~f:(fun (in1, in2, out, f2) ->
        if
          Int.min x y = Int.min in1 in2
          && Int.max x y = Int.max in1 in2
          && bf_equal f f2
        then Some out
        else None)
  ...
  and get_carrynode ({ gates; _ } as t) i =
    ...
    | None ->
        let res =
          if i = 0 then Wire.of_string "gct" |> Some
          else
...
let part1 s =
  ...
  System.section_decimal system 'z' |> Int.to_string |> Ok
...
let part2 s =
  ...
  Ok "i did this one with manual inspection"
