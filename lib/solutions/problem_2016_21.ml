...
let pop s i =
  ...
  let res =
    if i = 0 then String.drop_prefix s 1
    else if i + 1 = String.length s then String.drop_suffix s 1
...
let insert s i c =
  ...
  if i = 0 then c ^ s
  else if i = String.length s then s ^ c
...
module Instruction = struct
  ...
  let rev s x y =
    ...
    let john =
      if y + 1 = String.length s then "" else String.slice s (y + 1) 0
    in
  ...
  let of_string s =
    ...
    | "swap" -> (
        match words.(1) with
        | "position" ->
            Swap_pos (Int.of_string words.(2), Int.of_string words.(5))
...
let get_instructions s =
  String.split_lines s |> List.map ~f:Instruction.of_string

let part1 s =
  let instructions = get_instructions s in
  Instruction.scramble "abcdefgh" instructions |> Ok
