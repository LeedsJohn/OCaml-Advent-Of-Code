...
module State = struct
  ...
  let shield ({ mana; shield_time; mana_spent; _ } as t) =
    if mana < 113 || shield_time > 0
    then None
    else Some { t with mana = mana - 113; mana_spent = mana_spent + 113; shield_time = 6 }
  ;;

  let poison ({ mana; poison_time; mana_spent; _ } as t) =
    if mana < 173 || poison_time > 0
    then None
    else Some { t with mana = mana - 173; mana_spent = mana_spent + 173; poison_time = 6 }
  ;;

  let recharge ({ mana; recharge_time; mana_spent; _ } as t) =
    if mana < 229 || recharge_time > 0
    then None
    else
      Some { t with mana = mana - 229; mana_spent = mana_spent + 229; recharge_time = 5 }
  ;;
  ...
  let enemy_turn
    ...
    let enemy_health, poison_time =
      if poison_time > 0 then enemy_health - 3, poison_time - 1 else enemy_health, 0
    in
    ...
    let health, shield_time =
      if enemy_health <= 0
      then health, Int.max 0 (shield_time - 1)
  ...
  let my_turn
    ...
    let t = { t with mana; shield_time; poison_time; recharge_time; enemy_health } in
    match move t with
    | Some t -> Some { t with turn = false }
    | None -> None
  ;;
  ...
  let get_next_things t =
    ...
    | false -> [ enemy_turn t ]
    | true ->
      List.filter_map [ magic_missile; drain; shield; poison; recharge ] ~f:(fun move ->
        my_turn t move)
  ;;
...
let part1 s =
  ...
  djikstra start_state |> Int.to_string
;;

let part2 s =
  let start_state = State.of_string s in
  djikstra ~neighbor_states:State.get_next_things_hard start_state |> Int.to_string
;;
