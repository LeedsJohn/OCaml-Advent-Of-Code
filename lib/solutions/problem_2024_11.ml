...
let split_num n =
  ...
  [ Int.of_string (String.slice s 0 (l / 2)); Int.of_string (String.slice s (l / 2) 0) ]
;;

let get_next_stones n =
  if n = 0
  then [ 1 ]
  else if String.length (Int.to_string n) % 2 = 0
  then split_num n
  else [ n * 2024 ]
;;
  ...
  let rec count_for_stone n blinks =
    ...
    then 1
      ...
      | None ->
        let res =
          get_next_stones n
          |> List.sum (module Int) ~f:(fun n -> count_for_stone n (blinks - 1))
        in
...
let part2 s = stones_after_blinks (Parse.line_numbers s) 75 |> Int.to_string
