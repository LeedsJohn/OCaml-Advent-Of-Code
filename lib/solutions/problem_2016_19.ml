...
let get_last start_elves =
  let rec aux nums cur =
    if Set.length nums = 1
    then Set.nth nums 0 |> Option.value_exn
    else (
...
let get_last2 start_elves =
  let rec aux nums cur =
    ...
    then Set.nth nums 0 |> Option.value_exn
      ...
      let nums = Set.remove_index nums eliminated_idx in
      let next =
        if eliminated_idx < cur
        then get_next_idx nums (cur - 1)
...
let part1 s = get_last (Int.of_string s) |> Int.to_string
let part2 s = get_last2 (Int.of_string s) |> Int.to_string
