...
let count_characters s =
  let rec aux i = function
    ...
    | [ _ ] -> i + 1
    | a :: b :: c :: d :: tl ->
        if Char.(a = '\\' && b = 'x' && is_hex_digit c && is_hex_digit d) then
          aux (i + 1) tl
        else if Char.(a = '\\' && (b = '"' || b = '\\')) then
          aux (i + 1) (c :: d :: tl)
        else aux (i + 1) (b :: c :: d :: tl)
    | a :: b :: tl ->
        if Char.(a = '\\' && (b = '"' || b = '\\')) then aux (i + 1) tl
        else aux (i + 1) (b :: tl)
  in
...
let part1 s =
  ...
  total_chars - other_chars |> Int.to_string |> Ok
...
let part2 s =
  ...
  print_s [%message (total_chars : int) (other_chars : int)];
  other_chars - total_chars |> Int.to_string |> Ok
