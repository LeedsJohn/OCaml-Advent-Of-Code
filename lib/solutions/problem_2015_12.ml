...
module Json = struct
  type t = Ar of t list | Ob of t Map.M(String).t | S of string | N of int
  ...
  let of_string s =
    let rec get_thing i =
      ...
      | '"' -> get_str (i + 1)
      | c ->
          if Char.(c = '-' || is_digit c) then get_num i
  ...
  let rec get_stuff t =
    ...
    | Ob m ->
        if List.mem (Map.data m) (S "red") ~equal then 0
...
let part2 s =
  ...
  Json.get_stuff t |> Int.to_string |> Ok
