...
module State = struct
  type t = { a : int; b : int; ip : int; instructions : Instruction.t array }
  ...
  let step ({ a; b; ip; instructions } as t) =
    ...
    | Jie ('a', n) ->
        if a % 2 = 0 then { t with ip = ip + n } else { t with ip = ip + 1 }
    | Jie ('b', n) ->
        if b % 2 = 0 then { t with ip = ip + n } else { t with ip = ip + 1 }
    | Jio ('a', n) ->
        if a = 1 then { t with ip = ip + n } else { t with ip = ip + 1 }
    | Jio ('b', n) ->
        if b = 1 then { t with ip = ip + n } else { t with ip = ip + 1 }
    | instruction ->
        raise_s [%message "bad instruction" (instruction : Instruction.t)]
