...
module Snailfish = struct
  ...
  let explode_or_split t =
    let rec should_explode t depth =
      ...
      | _ ->
        (match t with
         | Pair (l, r) -> should_explode l (depth + 1) || should_explode r (depth + 1)
         | _ -> false)
    in
    ...
    in
    if should_explode t 0
    then Some `Explode
    else if should_split t
    then Some `Split
    else None
  ;;
  ...
  let split t =
    let rec aux t found =
      ...
      then t, true
        ...
        | Num n ->
          if n >= 10
          then Pair (Num (n / 2), Num ((n / 2) + (n % 2))), true
          else Num n, false
        | Pair (l, r) ->
  ...
  let explode t =
    ...
    let rec first_num_pos s i di =
      ...
      then None
      else if Char.is_digit (String.get s i) && not (Char.is_digit (String.get s (i - 1)))
      then Some i
      else first_num_pos s (i + di) di
    in
    let ep = explode_position 0 0 in
    let end_ep =
      String.findi s ~f:(fun i c -> i > ep && Char.equal c ']') |> Option.value_exn |> fst
    in
    let l, r =
      match of_string (String.slice s ep (end_ep + 1)) with
      | Pair (Num n1, Num n2) -> n1, n2
      | other -> raise_s [%message "Bad explode pair" ~snailfish_num:(other : t)]
    in
    ...
    let add_num s start john =
      ...
      String.slice s 0 start ^ Int.to_string (num + john) ^ String.slice s (start + l) 0
    in
    let right_start = first_num_pos s (ep + 1) 1 in
    let left_start = first_num_pos s (ep - 1) (-1) in
    let s =
      match right_start with
      | Some i -> add_num s i r
      | None -> s
    in
    let s =
      match left_start with
      | Some i -> add_num s i l
      | None -> s
    in
    of_string s
  ;;
...
let%expect_test "parse" =
  ...
  print_s
    [%sexp
      (Snailfish.of_string "[[[[1,3],[5,3]],[[1,3],[8,7]]],[[[4,9],[6,9]],[[8,2],[7,3]]]]"
       : Snailfish.t)];
  [%expect
    {|
       (Pair
