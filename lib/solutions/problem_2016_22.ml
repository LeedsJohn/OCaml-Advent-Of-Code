...
module Nodes = struct
  ...
  let viable_pair { board; _ } a_pos b_pos =
    if Coordinate.equal a_pos b_pos
    then false
    else (
  ...
  let move_data { board; goal_data_pos; empty_space = _ } start_pos end_pos =
    ...
    let board = Map.set board ~key:end_pos ~data:(b_used + a_used, b_avail - a_used) in
    if Coordinate.equal goal_data_pos start_pos
    then { board; goal_data_pos = end_pos; empty_space = start_pos }
  ...
  let get_neighbor_states (t : t) : t list =
    ...
    in
    Coordinate.neighbors t.empty_space |> List.map ~f:neighbors_from_pos |> List.join
  ;;
end
...
module Nodes_with_cost = struct
  module T = struct
    ...
    let get_cost ({ board; goal_data_pos = (x, y) as pos; empty_space } : Nodes.t) steps =
      let dist = x + y in
      let start_used, start_avail = Map.find_exn board (0, 0) in
      let goal_used, _ = Map.find_exn board (x, y) in
      if goal_used > start_used + start_avail
      then Int.max_value
      else (
...
let part2 s =
  ...
  in
  let _, res, _ =
    Nodes_with_cost.djikstra (Nodes_with_cost.get_cost nodes 0, 0, nodes) ~is_goal
  in
  Int.to_string res
;;
