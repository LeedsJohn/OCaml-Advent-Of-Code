...
let all_shortest_paths board start end_ =
  let start_pos =
    List.find_exn (Map.to_alist board) ~f:(fun (_, n) -> Char.equal n start) |> fst
  in
  let end_pos =
    List.find_exn (Map.to_alist board) ~f:(fun (_, n) -> Char.equal n end_) |> fst
  in
  let rec aux (level : Coordinate.t list list) =
    let ends_in_goal =
      List.filter level ~f:(fun path -> Coordinate.equal end_pos (List.hd_exn path))
    in
    if List.length ends_in_goal <> 0
    then List.map ends_in_goal ~f:List.rev
    else (
      let next_level =
        List.map level ~f:(fun path ->
          List.filter_map
            (Coordinate.neighbors (List.hd_exn path))
            ~f:(fun pos -> if Map.mem board pos then Some (pos :: path) else None))
        |> List.join
      in
      aux next_level)
  in
...
let rec shortest_path (start_pos : char) (end_pos : char) (r : int) (num_robots : int)
  : int
  =
  if r = num_robots
  then 1
    ...
    | None ->
      let all_paths =
        if r = 0
        then num_pad_shortest_paths start_pos end_pos
...
let part1 s =
  List.sum (module Int) (String.split_lines s) ~f:(fun line -> complexity line 3)
  |> Int.to_string
;;

let part2 s =
  List.sum (module Int) (String.split_lines s) ~f:(fun line -> complexity line 26)
  |> Int.to_string
;;
