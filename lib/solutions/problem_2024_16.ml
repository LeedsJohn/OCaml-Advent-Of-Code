...
let djikstra ?(end_positions = Set.empty (module Coordinate)) board start_positions =
  let visited = Hashtbl.create (module Cord2) in
  let pq = Pq.of_list (List.map start_positions ~f:(fun (pos, dir) -> pos, dir, 0)) in
  let rec aux pq =
    if Pq.is_empty pq
    then ()
    else (
      let (pos, dir, score), pq = Pq.get_exn pq in
      if Hashtbl.mem visited (pos, dir)
      then aux pq
      else (
        Hashtbl.add_exn visited ~key:(pos, dir) ~data:score;
        if Set.mem end_positions pos
        then ()
        else (
          let next_positions =
          ...
          let pq =
            List.fold next_positions ~init:pq ~f:(fun pq (pos, dir, score) ->
              ...
              | '#' -> pq
              | _ ->
                if Hashtbl.mem visited (pos, dir) then pq else Pq.add pq (pos, dir, score))
          in
          aux pq)))
  in
  aux pq;
...
let part1 s =
  ...
  let distances =
    djikstra ~end_positions:(Set.singleton (module Coordinate) ep) board [ sp, (1, 0) ]
  in
  List.find_map_exn Coordinate.offsets ~f:(fun dir -> Hashtbl.find distances (ep, dir))
  |> Int.to_string
;;
