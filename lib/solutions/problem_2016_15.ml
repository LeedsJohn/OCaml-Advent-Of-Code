...
let part1 s =
  ...
  first_start_time discs |> Int.to_string
;;

let part2 s =
  let discs = get_discs s in
  let discs =
    discs @ [ { num_positions = 11; start_position = 0; id = List.length discs + 1 } ]
  in
  first_start_time discs |> Int.to_string
;;
