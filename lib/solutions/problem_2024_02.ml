...
let rec is_increasing = function
  | a :: b :: tl ->
    if Int.between (b - a) ~low:1 ~high:3 then is_increasing (b :: tl) else false
  | _ -> true
;;
...
let is_increasing2 l =
  let rec aux left right =
    match right with
    | hd :: tl ->
      if is_increasing (List.rev_append left tl) then true else aux (hd :: left) tl
    | _ -> false
  in
  aux [] l
;;
...
let solve s inc_fn dec_fn =
  parse s |> List.count ~f:(fun line -> inc_fn line || dec_fn line) |> Int.to_string
;;
