...
let of_string s =
  let get_instruction_type line =
    if String.is_substring line ~substring:"toggle"
    then Toggle
    else if String.is_substring line ~substring:"off"
    then Off
    else On
  in
  ...
  |> List.map ~f:(fun line ->
    ...
    instruction_type, (Int.min x1 x2, Int.min y1 y2), (Int.max x1 x2, Int.max y1 y2))
;;
...
let apply_instruction board (instruction_type, (x1, y1), (x2, y2)) =
  ...
  List.iter
    (List.range x1 (x2 + 1))
    ~f:(fun x ->
      List.iter (List.range y1 (y2 + 1)) ~f:(fun y -> board.(y).(x) <- f board.(y).(x)))
;;
