...
let valid_update order_rules update =
  ...
  List.for_all pairs ~f:(fun (first, second) -> not (Set.mem order_rules (second, first)))
;;
...
let part1 s =
  ...
  List.sum
    ...
    updates
    ~f:(fun update ->
      if valid_update order_rules update
...
let part2 s =
  ...
  |> List.sum (module Int) ~f:(fun update -> List.nth_exn update (List.length update / 2))
  |> Int.to_string
;;
