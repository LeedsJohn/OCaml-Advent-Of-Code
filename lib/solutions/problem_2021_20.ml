...
module Game = struct
  ...
  let get { board; default; _ } pos = Map.find board pos |> Option.value ~default

  let get_alg_index t (x, y) =
    List.map
      (List.range (y - 1) (y + 2))
      ~f:(fun y -> List.map (List.range (x - 1) (x + 2)) ~f:(fun x -> get t (x, y)))
    |> List.join
    |> String.of_list
    |> bin_to_dec
  ;;
  ...
  let step ({ alg; default; board } as t) =
    ...
    in
    { t with board = new_board; default = (if Char.(default = '#') then '.' else '#') }
  ;;

  let of_string s =
    let alg = String.split_lines s |> List.hd_exn in
    let board = String.drop_prefix s (String.length alg + 1) |> Board.of_string in
    { alg; board; default = '.' }
  ;;
end
...
let part1 s =
  ...
  Map.count game.board ~f:(Char.equal '#') |> Int.to_string
;;

let part2 s =
  let game =
    List.fold (List.range 0 50) ~init:(Game.of_string s) ~f:(fun acc _ -> Game.step acc)
  in
  Map.count game.board ~f:(Char.equal '#') |> Int.to_string
;;
