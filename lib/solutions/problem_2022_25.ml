...
let _five_to_dec n = List.foldi n ~init:0 ~f:(fun i acc num -> (Int.pow 5 i * num) + acc)
  ...
  let res, carry =
    List.fold (List.rev n) ~init:([], 0) ~f:(fun (acc, carry) num ->
      ...
      let digit =
        ...
        | 1 -> '1'
        | 2 -> '2'
        | _ -> raise_s [%message "bad digit" ~digit:(num : int) ~carry:(next_carry : int)]
      in
...
let part1 s =
  ...
  let total =
    List.fold lines ~init:Z.zero ~f:(fun acc snafu -> Z.(acc + snafu_to_ten snafu))
  in
