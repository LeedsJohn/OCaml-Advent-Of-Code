...
module Room = struct
  ...
  let of_string s =
    ...
    let i = String.rfindi s ~f:(fun _ c -> Char.equal '-' c) |> Option.value_exn in
  ...
  let decode { name; id; checksum } =
    let name =
      String.map name ~f:(fun c -> if Char.equal c '-' then '-' else shift_letter c id)
    in
    { name; id; checksum }
  ;;
end
...
let part2 s =
  String.split_lines s
  |> List.map ~f:Room.of_string
  |> List.map ~f:Room.decode
  |> List.find_map_exn ~f:(fun { name; id; _ } ->
    if String.is_substring name ~substring:"northpole" then Some id else None)
  |> Int.to_string
;;
