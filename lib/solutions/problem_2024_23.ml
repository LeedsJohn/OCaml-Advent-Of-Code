...
module Lan_party = struct
  ...
  let get_connected_groups_of_n t n =
    let rec dfs cur points =
      ...
      then if is_all_connected t cur then [ cur ] else []
      else (
        match points with
        | [] -> []
    ...
    let rec aux cur = function
      ...
      | hd :: tl ->
        let group1 =
          ...
          if List.for_all cur ~f:(fun point -> Set.mem neighbors point)
          then aux (hd :: cur) tl
          else []
        in
...
let part2 s =
  ...
  List.sort group ~compare:String.compare |> String.concat ~sep:","
;;
