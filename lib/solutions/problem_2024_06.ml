...
module Game = struct
  type t = { board : bool Board.t; warden : Coordinate.t * Coordinate.t }
  ...
  let _to_string { board; warden = pos, _ } =
    let b =
      Map.map board ~f:(fun c -> if c then '#' else '.')
      |> Map.set ~key:pos ~data:'^'
    in
    Board.to_string b
  ...
  let does_loop t =
    ...
    let rec aux ({ warden = pos, dir; _ } as t) =
      if Hash_set.mem visited (pos, dir) then true
      else (
        Hash_set.add visited (pos, dir);
        match step t with None -> false | Some t -> aux t)
    in
    aux t
end
...
let part1 s =
  ...
  let rec aux ({ warden = pos, _; _ } as t : Game.t) =
    ...
    match Game.step t with None -> () | Some t -> aux t
  in
  aux game;
  Hash_set.length visited |> Int.to_string |> Ok
...
let part2 s =
  ...
  List.count spots_to_make_wall ~f:(fun spot ->
      let game =
        { game with board = Map.set game.board ~key:spot ~data:true }
      in
      Game.does_loop game)
  |> Int.to_string |> Ok
