...
let best_score ?(calories_count = Int.max_value) recipes num_ingredients =
  ...
  let rec aux cur calories recipes num_ingredients =
    ...
    then score_stuff cur calories
    else if calories > calories_count
    then 0
    else (
      match recipes with
      ...
      | (recipe, cals) :: tl ->
        ...
        |> List.map ~f:(fun i ->
          let next = List.map2_exn cur recipe ~f:(fun cur n -> cur + (i * n)) in
...
let part1 s =
  ...
  best_score recipes 100 |> Int.to_string
;;

let part2 s =
  let recipes = of_string s in
  best_score ~calories_count:500 recipes 100 |> Int.to_string
;;
