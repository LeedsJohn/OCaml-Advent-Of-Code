...
let biggest line ~num_digits =
  let rec aux acc start num_left =
    if num_left = 0
    then
      List.fold acc ~init:(0, 1) ~f:(fun (cur, mult) n -> cur + (n * mult), mult * 10)
      |> fst
    else (
      let next_val, next_i =
...
let part1 stuff =
  parse stuff |> List.sum (module Int) ~f:(biggest ~num_digits:2) |> Int.to_string
;;

let part2 stuff =
  parse stuff |> List.sum (module Int) ~f:(biggest ~num_digits:12) |> Int.to_string
;;
