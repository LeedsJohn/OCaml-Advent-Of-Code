...
functor
  ...
  struct
    ...
    let link (Node (x1, r1, c1) as t1 : tree) (Node (x2, r2, c2) as t2 : tree) : tree =
    ...
    let rec meld (Pq t1) (Pq t2) : t =
      ...
      | t1 :: ts1, t2 :: ts2 ->
        ...
        in
        if rank t1 < rank t2
        then aux t1 t2 ts1 ts2
        else if rank t2 < rank t1
        then aux t2 t1 ts2 ts1
    ...
    let get_exn (t : t) =
      ...
      let new_pq =
        match t with
        | Pq [] -> raise_s [%message "Attempted to get from empty priority queue"]
        | Pq ts ->
